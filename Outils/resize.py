...
def resize_and_rename_images_in_folder(folder_path, name, size=(40, 40)):
    ...
    for filename in os.listdir(folder_path):
        ...
        if os.path.isfile(file_path):
            try:
                ...
                new_filename = f"{name}-{image_counter}{file_extension}"
...
def rename_images_in_folder(folder_path, name):
    # Vérifier si le dossier existe
    if not os.path.exists(folder_path):
        print(f"Le dossier {folder_path} n'existe pas.")
        return
    ...
    for filename in os.listdir(folder_path):
        ...
        if os.path.isfile(file_path):
            try:
                ...
                print(f"Image {filename} renommée en {new_filename}.")
